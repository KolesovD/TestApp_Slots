...
namespace AxGrid.Tools.Binders{
	...
	public class UIButtonDataBind : Binder
	{
		...
		public const string BUTTON_ENABLE_PART = "Enable";
        public const string BUTTON_CLICK_PART = "Click";
		...
		public void awake()
		{
			...
			enableField = enableField == "" ? $"Btn{buttonName}{BUTTON_ENABLE_PART}" : enableField;
			if (!onKeyPress)
		...
		public void OnClick()
		{
			...
			if (!cancel)
			{
				...
				Model?.EventManager.Invoke($"On{buttonName}{BUTTON_CLICK_PART}");
			}
			cancel = false;
		}
